app.service('clientesService', [
	...
	this.getEnvase = function(id) {
		var d = $q.defer();
		$http.get("/clientes/envases/"+id).then(function(response) {
			d.resolve(response.data);
		}, function(response) {
			d.reject(response);
		});
...
app.controller("clientesController",[
	...
	'$window',
	'envaseService',
	function($scope, clientesService, $routeParams,$location,$window,envaseService){
	...
	$scope.showEnvase=false;
	$scope.quitarEnvases = function(data){
		$scope.showEnvase=true;
		$scope.envaseAdd = data;
	}
	$scope.devolEnvase=function(inf){
		$scope.showEnvase=false;
		envaseService.devolverEnvase($scope.idClient,inf.tipoEnvase,inf.cantidadEnvase).then(function(newCliente) {
			alert("Se han devuelto la cantidad de "+ inf.cantidadEnvase+" de "+inf.tipoEnvase);
			location.reload();
		});
		
	}
	$scope.verEnvases = function(id){
		$scope.idClient = id;
		clientesService.getEnvase(id).then(function(data) {
			$scope.envase = data;
			$('#mdlEnvase').modal('show');
		});
	}
