...
public class PDFFactura {
	 ...
	 * @throws IOException
	 */
	public Document construirPdf(Comprobante comprobante, String selloDigital, byte[] bytesQRCode, Estatus estatus, int tipo, Imagen imagen, List<Envase> envases, ClienteEnvases acumulados, DatosEmisor de)
		...
		construirBoceto(comprobante, tfd,tipo, estatus, imagen, envases, acumulados);
	...
	public Document construirPdf(Comprobante comprobante, Estatus estatus, Imagen imagen, List<Envase> envases, ClienteEnvases acumulados) throws DocumentException, MalformedURLException, IOException {

		construirBoceto(comprobante, null,0, estatus, imagen, envases, acumulados);
	...
	public Document construirPdfCancelado(Comprobante comprobante, String selloDigital, byte[] bytesQRCode,
			String selloCancelacion, Date fechaCancelacion,Imagen imagen, List<Envase> envases, ClienteEnvases acumulados)
		...
		construirBoceto(comprobante, tfd,0, Estatus.CANCELADO, imagen, envases, acumulados);
		construirTimbre(selloDigital, bytesQRCode, tfd);
	...
	private void agregarCeldaConFondo(String contenidoCelda, Font fuente, PdfPTable tabla, boolean centrado) {
		PdfPCell celda = new PdfPCell(new Paragraph(contenidoCelda, fuente));
		celda.setBorderWidth(1);
		celda.setBorderColor(new BaseColor(188,169,155));//(255,117,020));
		celda.setPadding(5);
		celda.setBackgroundColor(new BaseColor(188,169,155));
	...
	private void construirBoceto(Comprobante comprobante, TimbreFiscalDigital tfd,int encabezado, Estatus estatus, Imagen imagen, List<Envase> envases, ClienteEnvases acumulados)
			throws DocumentException, MalformedURLException, IOException {
...
//			 celdaLogo.addElement(chunkLogo);
			 ...
			 String rfc=comprobante.getEmisor().getRfc();
			 System.out.println("emisorrrrrrrrrrr:"+rfc);
			 if (rfc.equals("OIN980511H24")){
				 System.out.println("bbbbbbb");
				 imgLogo = Image.getInstance("images/logoOvani.jpg");
			 }
		//	imgLogo = Image.getInstance("images/okue.jpg");
			imgLogo.setScaleToFitHeight(false);
			imgLogo.scaleToFit(115F, 37.25F);
			Chunk chunkLogo = new Chunk(imgLogo, 10, -15);
			celdaLogo.addElement(chunkLogo);
		 }
...
//		 agregarChunkYNuevaLinea("OVANI", font1, fraseDatosEmisor);
		 ...
		 agregarChunkYNuevaLinea("Tel: (722) 222 2222", font3, fraseDatosEmisor);
		 agregarChunkYNuevaLinea("contact@ovani.com", font3, fraseDatosEmisor);
		...
		//ENVASES
		if(envases!=null && envases.size()>0) {
			PdfPTable tablaEnvases = new PdfPTable(2);
			tablaEnvases.setWidthPercentage(100);
			tablaEnvases.setWidths(new int[] { 10, 2 });
			PdfPCell celdaEnvases = new PdfPCell(new Paragraph("Envases de esta venta", fontHead));
			celdaEnvases.setBorderWidth(1);
			celdaEnvases.setBorderColor(new BaseColor(188,169,155));
			celdaEnvases.setPadding(5);
			celdaEnvases.setBackgroundColor(new BaseColor(188,169,155));
			celdaEnvases.setHorizontalAlignment(Element.ALIGN_CENTER);
			celdaEnvases.setColspan(3);
			tablaEnvases.addCell(celdaEnvases);
			for(Envase envase:envases) {
				this.agregarCelda(envase.getTipoEnvase(), font3, tablaEnvases, false);
				this.agregarCelda(envase.getCantidadEnvase()+"", font3, tablaEnvases, false);
			}
			PdfPTable tablaEnvasesPrincipal = new PdfPTable(3);
			this.crearNotaDoble(tablaEnvasesPrincipal, false, 48, 4, 48, tablaEnvases);
			document.add(tablaEnvasesPrincipal);
		}
		...
		if(acumulados!=null ) {
			...
			tablaEnvases.setWidthPercentage(100);
			tablaEnvases.setWidths(new int[] { 10, 2 });
			PdfPCell celdaEnvases = new PdfPCell(new Paragraph("Envases Acumulados", fontHead));
			celdaEnvases.setBorderWidth(1);
			celdaEnvases.setBorderColor(new BaseColor(188,169,155));
			celdaEnvases.setPadding(5);
			celdaEnvases.setBackgroundColor(new BaseColor(188,169,155));
			...
			celdaEnvases.setColspan(3);
			tablaEnvases.addCell(celdaEnvases);
			for(Envase envase:acumulados.getEnvases()) {
				this.agregarCelda(envase.getTipoEnvase(), font3, tablaEnvases, false);
				this.agregarCelda(envase.getCantidadEnvase()+"", font3, tablaEnvases, false);
			}
			PdfPTable tablaEnvasesPrincipal = new PdfPTable(3);
		...
		emptyCell.setBackgroundColor(new BaseColor(188,169,155));
		tablaImporteConLetra.addCell(emptyCell);
		...
		Phrase fraseLeyenda = new Phrase();
		// if (!estatus.equals(Estatus.GENERADO)) {
		Chunk chunkLeyenda = new Chunk("", fontTituloSellos);
		// fraseLeyenda.add(Chunk.NEWLINE);
		fraseLeyenda.add(chunkLeyenda);
		// }
		// fraseLeyenda.add(Chunk.NEWLINE);
